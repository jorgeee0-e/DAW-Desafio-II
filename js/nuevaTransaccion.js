...
const ingresos = (transacciones) => {
  const in_transacciones = transacciones.filter(
    (transaccion) => transaccion.tipo === "Ingresos"
  );
  let ingreso_total = 0;
...
const egresos = (transacciones) => {
  const in_transacciones = transacciones.filter(
    (transaccion) => transaccion.tipo === "Egresos"
  );
  let egreso_total = 0;
...
const nuevaTransaccion = (event) => {
  ...
  tipoTransaction =
    tipoTransaction.charAt(0).toUpperCase() + tipoTransaction.slice(1);
...
const label_refrsh = (transacciones) => {
  ...
  porcentajes.textContent =
    Math.abs(percent_gastos(transacciones).toFixed(0)) + "%";
};
...
const showTransactions = (transacciones) => {
  transacciones.forEach((transaction) => {
    console.log(
      `Tipo: ${transaction.tipo}, Descripción: ${transaction.descripcion}, Monto: ${transaction.monto}`
    );
  });
...
const addTo = (transacciones) => {
  ...
  if (btnIngresos.classList.contains("active")) {
    transacciones_filtered = transacciones.filter(
      (transaccion) => transaccion.tipo === "Ingresos"
    );
  } else {
    transacciones_filtered = transacciones.filter(
